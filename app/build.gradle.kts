...
android {
    ...
    defaultConfig {
        applicationId = "com.durcheins.apkupdater" + System.getenv("BUILD_TAG").orEmpty()
        minSdk = 26
        targetSdk = 36
                ...
                if (name == "package${variant.name.replaceFirstChar { it.uppercase() }}") {
                    doLast {
                        outputs.files.files
                            ...
                            .forEach { apk -> runCatching { apk.copyTo(File(apk.parentFile, "apkupdater-m3f-durcheins-${variant.buildType}.apk"), true) }.getOrNull() }
                    }
                }
            }
        }
